...
export async function GET() {
  ...
  try {
    ...
    for (let i = 0; i < statsRows.length && i < statsEnLabels.length; i++) {
      await run(`UPDATE beranda_stats_locales SET label = '${e(statsEnLabels[i])}' WHERE "_locale" = 'en' AND "_parent_id" = '${statsRows[i].id}'`);
    }
    ...
    for (let i = 0; i < heroFeatRows.length && i < heroFeatEnData.length; i++) {
      await run(`UPDATE beranda_hero_features_locales SET text = '${e(heroFeatEnData[i])}' WHERE "_locale" = 'en' AND "_parent_id" = '${heroFeatRows[i].id}'`);
    }
    ...
    for (let i = 0; i < partnersRows.length && i < partnersEnData.length; i++) {
      ...
      await run(`UPDATE beranda_partners_list_locales SET name = '${e(enName)}' WHERE "_locale" = 'en' AND "_parent_id" = '${partnersRows[i].id}'`);
    }
    ...
    for (let i = 0; i < ctaFeatRows.length && i < ctaFeatEnData.length; i++) {
      ...
      const existing = await run(`SELECT id FROM beranda_cta_features_locales WHERE "_locale" = 'en' AND "_parent_id" = '${ctaFeatRows[i].id}'`);
      if ((existing.rows as any[]).length > 0) {
        await run(`UPDATE beranda_cta_features_locales SET text = '${e(ctaFeatEnData[i])}' WHERE "_locale" = 'en' AND "_parent_id" = '${ctaFeatRows[i].id}'`);
      } else {
        await run(`INSERT INTO beranda_cta_features_locales (text, "_locale", "_parent_id") VALUES ('${e(ctaFeatEnData[i])}', 'en', '${ctaFeatRows[i].id}')`);
      }
    }
    ...
    const tkStatsEnLabels = ['Clients & Partners', 'Experts', 'Projects & Studies', 'Service Coverage', 'Trusted by Government & Private'];
    for (let i = 0; i < tkStatsRows.length && i < tkStatsEnLabels.length; i++) {
      await run(`UPDATE tentang_kami_stats_locales SET label = '${e(tkStatsEnLabels[i])}' WHERE "_locale" = 'en' AND "_parent_id" = '${tkStatsRows[i].id}'`);
    }
