...
export default async function JournalDetailPage({ params }: { params: Promise<RouteParams> }) {
  ...
  try {
    ...
    const relatedResult: any = await payload.find({
      ...
      where: publishedJournalWhere([{ id: { not_equals: journal.id } }, ...(categoryID ? [{ category: { equals: categoryID } }] : [])]),
  ...
  const schema = {
    ...
    author: authors.map((author: any) => ({ '@type': 'Person', name: author.name, affiliation: author.affiliation ? { '@type': 'Organization', name: author.affiliation } : undefined })),
    abstract: richTextToPlain(journal.abstract),
    keywords: Array.isArray(journal.keywords) ? journal.keywords.map((keyword: any) => keyword.keyword || keyword).filter(Boolean).join(', ') : undefined,
    ...
    <>
      ...
      <main style={{ paddingTop: 120, minHeight: '100vh', background: '#f8fafc', paddingBottom: 80 }}>
        <div className="container" style={{ maxWidth: 980 }}>
          ...
          <article className="card" style={{ padding: 'clamp(28px, 5vw, 52px)', borderRadius: 24, boxShadow: '0 10px 40px -10px rgba(0,0,0,0.06)', border: '1px solid #e2e8f0', background: '#ffffff' }}>
            <div className={cover ? 'journal-detail-hero journal-detail-hero--with-cover' : 'journal-detail-hero'} style={{ display: 'grid', gap: '32px', alignItems: 'center' }}>
              <div>
                <span className="badge badge-primary" style={{ fontSize: '0.85rem', padding: '6px 14px', borderRadius: '100px', fontWeight: 600 }}>
                  {typeof journal.category === 'object' && journal.category?.name ? journal.category.name : (isEn ? 'Publication' : 'Publikasi')}
                </span>
                <h1 style={{ color: '#0f172a', lineHeight: 1.25, fontSize: 'clamp(1.5rem, 3.5vw, 2.1rem)', margin: '18px 0 20px 0', fontWeight: 700 }}>
                  {journal.title}
                </h1>
                <div style={{ display: 'flex', flexWrap: 'wrap', gap: 16, color: '#64748b', fontSize: '0.92rem', alignItems: 'center' }}>
                  <span>
                    <Calendar size={16} style={{ verticalAlign: '-3px', marginRight: 6, color: '#2563eb' }} />
                    {new Date(journal.publishedAt || journal.createdAt).toLocaleDateString(dateLocale, { year: 'numeric', month: 'long', day: 'numeric' })}
                  </span>
                  {journal.publicationYear && <span>Tahun {journal.publicationYear}</span>}
                  {journal.doi && (
                    <a href={`https://doi.org/${journal.doi}`} rel="noreferrer" target="_blank" style={{ color: '#2563eb', fontWeight: 600, textDecoration: 'none' }}>
                      DOI: {journal.doi}
                    </a>
                  )}
                </div>
              ...
              {cover && (
                <div style={{ position: 'relative', width: '100%', aspectRatio: '3 / 4', overflow: 'hidden', borderRadius: 16, background: '#f1f5f9', border: '1px solid #cbd5e1', boxShadow: '0 12px 30px -8px rgba(0,0,0,0.18)' }}>
                  <img
            ...
            <section style={{ marginTop: 40, paddingTop: 32, borderTop: '1px solid #f1f5f9' }}>
              <h3 style={{ color: '#0f172a', fontSize: '1.2rem', marginBottom: 16, fontWeight: 700 }}>{isEn ? 'Authors' : 'Penulis'}</h3>
              <div style={{ display: 'grid', gap: 16 }}>
                {authors.length ? (
                  authors.map((author: any, index: number) => (
                    <div key={author.id || index} style={{ display: 'flex', gap: 14, color: '#334155', alignItems: 'flex-start', background: '#f8fafc', padding: 16, borderRadius: 12, border: '1px solid #f1f5f9' }}>
                      ...
                      <div>
                        ...
                        {author.affiliation && <div style={{ fontSize: '0.9rem', color: '#64748b', marginTop: 2 }}>{author.affiliation}</div>}
                      </div>
                    </div>
                  ))
                ) : (
            ...
            <section className="article-content" style={{ marginTop: 36 }}>
              <h3 style={{ color: '#0f172a', fontSize: '1.25rem', marginBottom: 14, fontWeight: 700 }}>{isEn ? 'Abstract' : 'Abstrak'}</h3>
              <RichTextRenderer content={journal.abstract} />
              {journal.content && (
                <>
                  <h3 style={{ color: '#0f172a', fontSize: '1.25rem', marginTop: 32, marginBottom: 14, fontWeight: 700 }}>{isEn ? 'Full discussion' : 'Pembahasan'}</h3>
            ...
            {document && (
              <section style={{ marginTop: 36, background: 'linear-gradient(135deg, #eff6ff 0%, #f0fdf4 100%)', border: '1px solid #bfdbfe', padding: 24, borderRadius: 16, display: 'flex', gap: 16, alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap' }}>
                <div style={{ display: 'flex', gap: 14, alignItems: 'center' }}>
                  <div style={{ width: 48, height: 48, borderRadius: 12, background: '#ffffff', color: '#2563eb', display: 'flex', alignItems: 'center', justifyContent: 'center', boxShadow: '0 2px 8px rgba(0,0,0,0.06)' }}>
                    <FileText size={24} />
                  </div>
                  <div>
                    <strong style={{ color: '#1e3a8a', fontSize: '1.05rem', display: 'block' }}>{isEn ? 'Full journal document' : 'Dokumen Jurnal Lengkap'}</strong>
                    <span style={{ color: '#64748b', fontSize: '0.88rem' }}>Format PDF Resmi</span>
                  </div>
                </div>
                <a className="btn btn-primary" download href={document} style={{ padding: '12px 24px', borderRadius: 10, fontWeight: 600, display: 'inline-flex', alignItems: 'center', gap: 8 }}>
            ...
            {(journal.volume || journal.issue || journal.pages || journal.issn) && (
              <section style={{ marginTop: 36 }}>
                <h3 style={{ color: '#0f172a', fontSize: '1.2rem', marginBottom: 16, fontWeight: 700 }}>{isEn ? 'Publication details' : 'Detail Publikasi'}</h3>
                <dl style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(160px, 1fr))', gap: 14, margin: 0 }}>
                  {[
                    ...
                    .filter(([, value]) => value)
                    .map(([label, value]) => (
                      <div key={String(label)} style={{ background: '#f8fafc', borderRadius: 12, padding: 16, border: '1px solid #f1f5f9' }}>
                        <dt style={{ color: '#64748b', fontSize: '0.85rem', fontWeight: 600 }}>{label}</dt>
                        <dd style={{ margin: '6px 0 0', color: '#0f172a', fontWeight: 700, fontSize: '0.98rem' }}>{value}</dd>
                      </div>
                    ))}
                </dl>
              </section>
            ...
            <section style={{ marginTop: 36 }}>
              <h3 style={{ color: '#0f172a', fontSize: '1.2rem', marginBottom: 14, fontWeight: 700 }}>{isEn ? 'Citation' : 'Sitasi'}</h3>
              <p style={{ padding: 18, borderRadius: 12, background: '#f8fafc', color: '#334155', border: '1px solid #e2e8f0', fontSize: '0.92rem', lineHeight: 1.6 }}>
          ...
          {related.length > 0 && (
            <section style={{ marginTop: 48 }}>
              <h3 style={{ color: '#0f172a', fontSize: '1.4rem', marginBottom: 20, fontWeight: 700 }}>{isEn ? 'Related journals' : 'Jurnal Terkait'}</h3>
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(240px, 1fr))', gap: 20 }}>
                {related.map((item: any) => (
                  <Link className="card" href={journalHref(resolved.locale, item.slug || item.id)} key={item.id} style={{ padding: 20, textDecoration: 'none', borderRadius: 16, border: '1px solid #e2e8f0' }}>
                    <strong style={{ color: '#0f172a', fontSize: '1.02rem', lineHeight: 1.4, display: 'block', marginBottom: 8 }}>{item.title}</strong>
                    <p style={{ color: '#64748b', fontSize: '0.88rem', margin: 0 }}>{journalAuthors(item).map((author: any) => author.name).join(', ')}</p>
                  </Link>
                ))}
              </div>
            </section>
