          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-6">
            {coreValuesData.map((cv: any, index: number) => {
              ...
              return (
                <div 
                  key={cv.name + index} 
                  className={`group relative p-6 bg-white rounded-[1.5rem] border border-[#e5e7eb] shadow-sm overflow-hidden h-full min-h-[260px] flex flex-col justify-center hover:shadow-md transition-all duration-300 ${isLow ? 'lg:translate-y-8' : ''}`}
                >
                  ...
                  <div className="relative z-10 flex flex-col">
                    ...
                    <h4 className="text-lg font-bold text-[#111827] mb-2">{cv.name}</h4>
                    <p className="text-[14px] text-[#4b5563] leading-relaxed">{cv.desc}</p>
                  </div>
                </div>
              );
            })}
