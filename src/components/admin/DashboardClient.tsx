...
function MetricCard({ label, value, detail, href, icon, tone = 'blue', loading }: { label: string; value?: number; detail: string; href: string; icon: string; tone?: string; loading: boolean }) {
  return <a className={`mwc-metric mwc-metric--${tone}`} href={href} aria-label={label}>
...
function AttentionPanel({ data, isEn }: { data: DashboardData; isEn: boolean }) {
  const items = [
    { count: data.stats.articles.draft, label: isEn ? 'draft articles' : 'artikel draft', href: '/admin/collections/articles?where[status][equals]=draft', icon: 'article' },
    { count: data.stats.journals.draft, label: isEn ? 'draft journals' : 'jurnal draft', href: '/admin/collections/journals?where[status][equals]=draft', icon: 'menu_book' },
    { count: data.stats.contacts.recentCount, label: isEn ? 'new messages' : 'pesan baru', href: '/admin/collections/contact-submissions', icon: 'mail' },
  ...
  return (
    <section className="mwc-panel mwc-attention" aria-labelledby="attention-title">
      ...
      {items.length ? (
        ...
        <p className="mwc-empty">{isEn ? 'No urgent actions required at this time.' : 'Tidak ada tindakan mendesak saat ini.'}</p>
      )}
    </section>
  )
}
        ...
        <div className="mwc-activity__list">
          {Array.from({ length: 4 }, (_, index) => (
            <div className="mwc-activity__skeleton" key={index}>
              ...
              <span><i className="mwc-skeleton" /><i className="mwc-skeleton" /></span>
            </div>
          ))}
        </div>
...
export const DashboardClient: React.FC = () => {
  ...
  const actionItems = useMemo(() => [
    { href: '/admin/collections/articles/create', icon: 'post_add', label: isEn ? 'New Article' : 'Artikel Baru', primary: true },
    { href: '/admin/collections/journals/create', icon: 'note_add', label: isEn ? 'New Journal' : 'Jurnal Baru', primary: true },
  ...
  useEffect(() => {
    ...
    const load = async () => {
      try {
        ...
        if (!response.ok) throw new Error(isEn ? `Failed to load dashboard (${response.status})` : `Dashboard tidak dapat dimuat (${response.status})`)
        ...
        if (!controller.signal.aborted) {
          setError(cause instanceof Error ? cause.message : (isEn ? 'Failed to load dashboard' : 'Dashboard tidak dapat dimuat'))
        }
      } finally {
  ...
  const subtitle = error 
    ? (isEn ? 'Data unavailable. Please try reloading.' : 'Data belum tersedia. Silakan coba muat ulang.')
    : loading 
    ? (isEn ? 'Preparing your operational summary…' : 'Menyiapkan ringkasan operasional Anda…')
    : data?.stats.contacts.recentCount 
    ? (isEn ? `There are ${data.stats.contacts.recentCount} new contact messages in the last 30 days.` : `Ada ${data.stats.contacts.recentCount} pesan kontak baru dalam 30 hari terakhir.`)
    : (isEn ? 'Summary of your website content and activities.' : 'Ringkasan konten dan aktivitas website Anda.')
    ...
    <main className="mwc-dashboard">
      <div className="mwc-dashboard__canvas">
        ...
        <section className="mwc-metrics" aria-label={isEn ? 'Content summary' : 'Ringkasan konten'}>
          <MetricCard
            detail={data ? `${data.stats.articles.published} published · ${data.stats.articles.draft} draft` : (isEn ? 'Loading status…' : 'Memuat status…')}
          ...
          />
          <MetricCard
            detail={data ? `${data.stats.journals.published} published · ${data.stats.journals.draft} draft` : (isEn ? 'Loading status…' : 'Memuat status…')}
          ...
          />
          <MetricCard
            detail={data ? `${data.stats.contacts.recentCount} ${isEn ? 'new in 30 days' : 'baru 30 hari ini'}` : (isEn ? 'Loading status…' : 'Memuat status…')}
          ...
          />
          <MetricCard
            detail={data ? `${data.stats.media.total} ${isEn ? 'files in library' : 'file di library'}` : (isEn ? 'Loading status…' : 'Memuat status…')}
                  ...
                  <div className="mwc-chart__area">
                    <ResponsiveContainer height="100%" width="100%">
                      <AreaChart data={data.weeklyChartData} margin={{ top: 8, right: 8, left: -20, bottom: 0 }}>
                        ...
                        <XAxis axisLine={false} dataKey="name" tick={{ fill: '#687087', fontSize: 11 }} tickLine={false} />
                        <YAxis allowDecimals={false} axisLine={false} tick={{ fill: '#687087', fontSize: 11 }} tickLine={false} />
                        <Tooltip />
                        <Area dataKey="articles" fill="url(#mwc-content-growth)" name={isEn ? 'Articles' : 'Artikel'} stroke="#2563eb" strokeWidth={2.5} type="monotone" />
                        <Area dataKey="journals" fill="none" name={isEn ? 'Journals' : 'Jurnal'} stroke="#7c3aed" strokeWidth={2.5} type="monotone" />
                      </AreaChart>
                    </ResponsiveContainer>
                  </div>
                ) : (
                  <p className="mwc-empty">{isEn ? 'No growth data available to display.' : 'Belum ada data pertumbuhan untuk ditampilkan.'}</p>
                )}
              </section>
